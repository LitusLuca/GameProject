...
void EntityBase::checkCollision()
{
	...
	for (int x = fromX; x <= toX; ++x)
	{
		for (int y = fromY; y <= toY; ++y)
		{
			...
			CollisionElement e(area, tile->m_proporties, tileBounds);
			m_collisions.emplace_back(e);
			if (tile->m_warp && m_type == EntityType::Player) gameMap->loadNext();
		}
	}
}
	...
	if (!m_collisions.empty())
	{
		...
		for (auto& itr : m_collisions)
		{
			...
			else
			{
				...
				{
					resolve = itr.m_tileBounds.top - (m_AABB.top + m_AABB.height);
				}
				move(0, resolve);
				m_velocity.y = 0;
